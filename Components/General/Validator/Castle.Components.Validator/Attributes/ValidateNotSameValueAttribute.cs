    ...
    public class ValidateNotSameValueAttribute : AbstractValidationAttribute
    {
            ...
            : base(errorMessage)
        {
            ...
            if (typeof(IConvertible).IsAssignableFrom(valueType))
                this.value = Convert.ChangeType(mustNotBeThisValue, valueType);
            else
            {
                MethodBase createInstance = valueType.GetMethod("Parse",BindingFlags.Public|BindingFlags.Static);
                if(createInstance!=null)
                {
                    this.value = createInstance.Invoke(null, new object[] { mustNotBeThisValue });
                }
                ConstructorInfo ctor = valueType.GetConstructor(new Type[] { mustNotBeThisValue.GetType() });
                if(ctor==null)
                {
                    throw new ArgumentException(
                        "valueType must be a type that implements IConvertible, or have a Parse method, or have a constructor that accept a " + mustNotBeThisValue.GetType()
                        );
                }
                this.value = Activator.CreateInstance(valueType, mustNotBeThisValue);
            }
        }
