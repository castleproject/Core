...
namespace Castle.Igloo.Configuration
{
	...
	/// IglooConfigSettings.
	/// </summary>
	public class IglooConfigSettings
	{
		#region Const
		/// <summary>
        ...
        private const string NODE_COMMANDS_XPATH = "action-mappings/actions";
        private const string NODE_GLOBAL_COMMANDS_XPATH = "global-actions/action";
		...
		/// <summary>
		/// Creates IglooConfigSettings from an XmlNode read of the web.config and an IFormatProvider.
		/// </summary>
		/// <param name="configNode">The XmlNode from the configuration file.</param>
		/// <param name="formatProvider">The provider.</param>
		public IglooConfigSettings(XmlNode configNode, IFormatProvider formatProvider) 
		{
		...
		private void LoadWebViews(XmlNode configNode, IFormatProvider formatProvider)
		{
			...
			foreach( XmlNode viewNode in configNode.SelectNodes( NODE_WEB_VIEW_XPATH ) )
			{
				...
				{
                    throw new ConfigurationErrorsException(Resource.ResourceManager.FormatMessage(Resource.MessageKeys.ViewAlreadyConfigured, viewName)); 
				}
			}
		}
		...
		private void LoadWinViews(XmlNode configNode, IFormatProvider formatProvider)
		{
			...
			foreach( XmlNode viewNode in configNode.SelectNodes( NODE_WIN_VIEW_XPATH ) )
			{
				...
				{
                    throw new ConfigurationErrorsException(Resource.ResourceManager.FormatMessage(Resource.MessageKeys.ViewAlreadyConfigured, viewId)); 
				}
			}
		}
		...
		public virtual string GetNextView(string viewID, string commandID)
		{
            ...
            if (commandID!=null && _globalCommands.ContainsKey(commandID))
			{
			...
			else
			{
				...
				{
                    throw new ConfigurationErrorsException(Resource.ResourceManager.FormatMessage(Resource.MessageKeys.CantFindCommandMapping, viewID)); 
				}
				nextView = setting[commandID];
				if( nextView==null )
				{
                    throw new ConfigurationErrorsException(Resource.ResourceManager.FormatMessage(Resource.MessageKeys.CantGetNextView, viewID, commandID)); 
				}				
			}
