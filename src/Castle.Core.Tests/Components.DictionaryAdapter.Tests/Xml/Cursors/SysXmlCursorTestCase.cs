	...
	public class SysXmlCursorTestCase
	{
		...
		public void Constructor_RequiresNode()
		{
			Assert.Throws<ArgumentNullException>(() =>
				new SysXmlCursor(null, ItemType, CursorFlags.Elements));
		}
		...
		{
			var xml    = Xml("<X/>");
			var cursor = new SysXmlCursor(xml, ItemType, CursorFlags.AllNodes);
		...
		{
			var xml    = Xml("<X A='a' Item='2' B='b'> <A/> <Item>1</Item> <B/> </X>");
			var cursor = new SysXmlCursor(xml, ItemType, CursorFlags.AllNodes | CursorFlags.Multiple);
		...
		public void Iterate_Element_WhenNoMatchExists()
		{
			...
			var cursor = new SysXmlCursor(xml, ItemType, CursorFlags.Elements);
		...
		{
			var xml    = Xml("<X Item='-'> <A/> <Item>1</Item> <B/> <Item>2</Item> <C/> </X>");
			var cursor = new SysXmlCursor(xml, ItemType, CursorFlags.Elements | CursorFlags.Multiple);
		...
		public void Iterate_Attribute_WhenNoMatchExists()
		{
			...
			var cursor = new SysXmlCursor(xml, ItemType, CursorFlags.Attributes);
		...
		public void MoveTo_RecognizedNode_Succeeds_ForElement()
		{
			...
			var cursor = new SysXmlCursor(xml, ItemAndOtherType, CursorFlags.Elements | CursorFlags.Multiple);
		...
		public void MoveTo_RecognizedNode_Succeeds_ForAttribute()
		{
			...
			var cursor = new SysXmlCursor(xml, ItemAndOtherType, CursorFlags.Attributes | CursorFlags.Multiple);
		...
		public void AsVirtual_WhenParentIsRealNode()
		{
			var xml = Xml("<X/>");
			IXmlCursor cursor;
			cursor = xml.SelectChildren(ItemType, CursorFlags.Elements | CursorFlags.Mutable);
			cursor.MoveNext();
		...
		public void AsVirtual_WhenParentIsVirtualNode()
		{
			...
			cursor = xml.SelectChildren(ItemType, CursorFlags.Elements | CursorFlags.Mutable);
			cursor.MoveNext();
			cursor = cursor.SelectChildren(OtherType, CursorFlags.Elements | CursorFlags.Mutable);
			cursor.MoveNext();
			var node = (IXmlNode) cursor;

			node.Value = "1";

			Assert.That(xml.GetNode(), XmlEquivalent.To("<X> <Item><Other>1</Other></Item> </X>"));
		}
		...
		public void Create_BeforeEnd_IsInsert_ForElement()
		{
			...
			var cursor = new SysXmlCursor(xml, ItemAndOtherType, CursorFlags.Elements);
			cursor.MoveNext();
		...
		public void Create_BeforeEnd_IsInsert_ForAttribute()
		{
			...
			var cursor = new SysXmlCursor(xml, ItemAndOtherType, CursorFlags.Attributes);
