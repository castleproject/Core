...
namespace Castle.DynamicProxy.Tests
{
	...
	public class ClassProxyConstructorsTestCase : BasePEVerifyTestCase
	{
		...
		{
			var exception = Assert.Throws<ArgumentException>(() => generator.CreateClassProxy(typeof(List<>), new IInterceptor[0]));
			StringAssert.StartsWith(
				"Can not create proxy for type System.Collections.Generic.List`1 because it is an open generic type.",
				exception.Message);
		}
		...
		public void Cannot_proxy_generic_type_with_open_generic_type_parameter()
		{
			...
			var ex = Assert.Throws<ArgumentException>(() => generator.CreateClassProxy(targetType, new IInterceptor[0]));
			StringAssert.StartsWith(
		...
		{
			var ex = Assert.Throws<ArgumentException>(() => generator.CreateClassProxy(typeof(PrivateClass), new IInterceptor[0]));
			StringAssert.StartsWith(
		...
		{
			var ex = Assert.Throws<ArgumentException>(() => generator.CreateClassProxy(typeof(List<List<PrivateClass>>), new IInterceptor[0]));
