...
namespace Castle.DynamicProxy.Tests
{
	...
	public class MixinTestCase : BasePEVerifyTestCase
	{
		...
		public void WillCacheTypesWithMixins()
		{
			...
			ProxyGenerationOptions proxyGenerationOption2 = new ProxyGenerationOptions();
			proxyGenerationOption2.AddMixinInstance(new SimpleMixin());
			object proxy2 = generator.CreateClassProxy(
				typeof(object), proxyGenerationOption2, new AssertInvocationInterceptor());
		...
		public void WillCacheMixinsOfSameTypesRegisteredAtDifferentOrders()
		{
			...
			proxyGenerationOptions.AddMixinInstance(new SimpleMixin());
			proxyGenerationOptions.AddMixinInstance(new OtherMixin());

			ProxyGenerationOptions proxyGenerationOptions2 = new ProxyGenerationOptions();

			proxyGenerationOptions2.AddMixinInstance(new OtherMixin());
			proxyGenerationOptions2.AddMixinInstance(new SimpleMixin());

			AssertInvocationInterceptor interceptor2 = new AssertInvocationInterceptor();
			AssertInvocationInterceptor interceptor = new AssertInvocationInterceptor();

			object proxy2 = generator.CreateClassProxy(
				typeof(SimpleClass), proxyGenerationOptions2, interceptor);

			object proxy = generator.CreateClassProxy(
				typeof(SimpleClass), proxyGenerationOptions, interceptor2);
