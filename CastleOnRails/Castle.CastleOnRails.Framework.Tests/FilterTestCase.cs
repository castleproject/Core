	...
	public class FilterTestCase
	{
		...
		public void FilteringAndSkippingFiltering()
		{
			...
			RailsEngineContextImpl context = new 
				RailsEngineContextImpl("/myvirdir/section/filtered/index.rails");
			_engine.Process( context );
			Assert.AreEqual( "index view contents", context.Output );

			// Filtered action
			context = new RailsEngineContextImpl("/myvirdir/section/filtered/save.rails");
			_engine.Process( context );
			Assert.AreEqual( "(before)(after)save view contents", context.Output );

			// Filtered action
			context = new RailsEngineContextImpl("/myvirdir/section/filtered/update.rails");
			_engine.Process( context );
		...
		public void BeforeFilters()
		{
			...
			// Non filtered action
			RailsEngineContextImpl context = new 
				RailsEngineContextImpl("/myvirdir/section/filtered2/index.rails");
			_engine.Process( context );
			Assert.AreEqual( "index view contents", context.Output );

			// Filtered action
			context = new RailsEngineContextImpl("/myvirdir/section/filtered2/save.rails");
			_engine.Process( context );
			Assert.AreEqual( "(before)save view contents", context.Output );

			// Filtered action
			context = new RailsEngineContextImpl("/myvirdir/section/filtered2/update.rails");
			_engine.Process( context );
