...
//			Kernel.AddCustomComponent( model );
			...
			Kernel.ComponentModelCreated += new ComponentModelDelegate(OnNewModel);
		}

		private void OnNewModel(ComponentModel model)
		{
			// We check if the component being registered is an ActiveRecord

			if ( typeof(ActiveRecordBase).IsAssignableFrom( model.Implementation ) )
			{
